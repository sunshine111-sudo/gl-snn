...
class Token_QK_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        power_list.append(calculate_spike_rate(x_for_qkv))
        ...
        power_list.append(calculate_spike_rate( attn))
        x = torch.mul(q,attn)

        x = x.flatten(2, 3)
        power_list.append(calculate_spike_rate(x))
...
class PatchEmbedInit(nn.Module):
    ...
    def forward(self, x):
        ...
        x_feat = x
        power_list.append(calculate_spike_rate(x_feat))
...
class SSCFM(nn.Module):
    ...
    def forward(self, x, y):
        ...
        # fuse = self.proj_bn(fuse.flatten(0, 1))
        # fuse = self.proj_conv(fuse)
        fuse = self.proj_lif2(fuse)
        power_list.append(calculate_spike_rate(fuse))
...
class attentionFusion(nn.Module):
    ...
    def forward(self, x, y):
        ...
        # avg = self.proj_lif(fuse)
        # max = self.proj_lif1(fuse)
        # power_list.append(calculate_spike_rate(avg))
        # power_list.append(calculate_spike_rate(max))
        ...
        # avg = self.proj_lif(avg)
        # avg=self.conv1(avg)
        # avg=self.bn1(avg).reshape(T, B, -1, 1, 1).contiguous()
        # avg=self.proj_lif1(avg).flatten(0, 1)
        # avg_out=self.conv2(avg)
        #
        # max = self.proj_lif(max)
        # max = self.conv1( max )
        # max = self.bn1( max ).reshape(T, B, -1, 1, 1).contiguous()
        # max = self.proj_lif1( max ).flatten(0, 1)
        # max_out = self.conv2( max )
        out = avg_out + max_out
...
class localBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # x_out=self.proj_res_lif(x_out)
        # x_out = self.proj_lif3(x_out)
        # feat = self.proj_res_conv(feat.flatten(0, 1))
        # feat = self.proj_res_bn(feat).reshape(T, B, -1, H, W).contiguous()
        # feat = self.proj_res_lif(feat)
...
class spiking_transformer(nn.Module):
    def __init__(self,
        ...
        # self.fuseAFF =AFF(embed_dims=embed_dims)
        # self. CAS2FF =      CAS2F(embed_dims=embed_dims)
    ...
    def forward_features(self, x):
        ...
        # x=self.CAS2FF(x_1, x_2)
        # feat=self.local(feat)
        # x = x_1+x_2
        ...
        # print(x)
        # print(local)
        # # x = self.atfuse(x_1, x_2)
        # # x = self.fuse1(x_1, x_2)x
        # x=x+local
        x = self.atfuse(x, local)
        # x=local

        # x=self.fuseAFF(x_1,x_2)
        x=x.mean(0)
...
def calculate_spike_rate(tensor):
    """
    计算输入张量中1的比例（脉冲发射率）
    参数:
        tensor: 输入张量（任意维度）
    返回:
        spike_rate: 1的比例（0.0~1.0）
    """
    if not isinstance(tensor, torch.Tensor):
        tensor = torch.tensor(tensor)

    # 统计1的数量（假设张量是二值化的0/1）
    ones_count = torch.sum(tensor == 1).float()
    total_elements = tensor.numel()

    spike_rate = ones_count / total_elements
    return spike_rate.item()
